...
function InfoPanel({item, color, light, onClose, t}) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:300,background:"rgba(28,24,20,0.65)",display:"flex",alignItems:isDesktop?"center":"flex-end",justifyContent:"center",backdropFilter:"blur(4px)",padding:isDesktop?"24px":0}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"white",borderRadius:isDesktop?"20px":"20px 20px 0 0",padding:isDesktop?"24px 32px 36px":"8px 20px 44px",maxWidth:isDesktop?640:580,width:"100%",boxShadow:isDesktop?"0 24px 80px rgba(0,0,0,0.25)":"0 -12px 60px rgba(0,0,0,0.2)",maxHeight:"90vh",overflowY:"auto"}}>
        {!isDesktop && <div style={{width:32,height:4,borderRadius:2,background:"#DDD",margin:"14px auto 16px"}}/>}
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(248,244,238,0.96)",backdropFilter:"blur(8px)",borderBottom:"1px solid "+mist}}>
        <div style={{maxWidth:isDesktop?1200:620,margin:"0 auto",display:"flex",alignItems:"center",padding:isDesktop?"0 32px":"0 10px"}}>
          ...
          <div style={{display:"flex",flex:1,overflowX:"auto",gap:isDesktop?4:0}}>
            {tabs.map(tt=>(
              <button key={tt.id} onClick={()=>setTab(tt.id)} style={{padding:isDesktop?"16px 14px 14px":"11px 9px 9px",background:"none",border:"none",cursor:"pointer",borderBottom:"2px solid "+(tab===tt.id?ember:"transparent"),transition:"border-color 0.2s",whiteSpace:"nowrap",flexShrink:0}}>
